...
$sql = "SELECT id, title, subtitle, category, img, created_at FROM news ORDER BY created_at DESC";
...
<head>
    ...
    <title>1000 Hills Rugby | News Page</title>
   ...
   <div class="relative lg:hidden flex flex-wrap items-center">
     ...
     >
       ...
       <li class="relative">
        ...
        <div class="grid grid-cols-1 mt-[80px]  md:grid-cols-3 gap-4">
            <?php if ($result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <div class="bg-white rounded-lg  shadow-md  overflow-hidden">
                        <!-- Use the img path directly from the database -->
                        <img src="<?php echo htmlspecialchars($row['img']); ?>" class="w-full h-50 object-cover" alt="">
                        <div class="p-4">
                            <?php if (!empty($row['category'])): ?>
                                <p class="text-sm text-gray-500"><?php echo htmlspecialchars($row['category']); ?></p>
                            <?php endif; ?>
                            ...
                            <p class="text-sm text-gray-500"><?php echo date("l, d F, Y", strtotime($row['created_at'])); ?></p>
                            <a href="news-detail.php?id=<?php echo $row['id']; ?>" class="text-blue-500 hover:text-blue-700">Read more →</a>
                        </div>
                    </div>
                <?php endwhile; ?>
            <?php else: ?>
                <p class="text-center text-red-500">No news available.</p>
            <?php endif; ?>
        </div>
    </div>
</body>
